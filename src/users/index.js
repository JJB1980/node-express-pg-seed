...
async function validateEmail (request, response) {
  ...
  try {
    const result = await dataApi(actions.USER_VALIDATE_EMAIL, [email], request);
    if (result.length) {
...
async function signup (request, response) {
  ...
  try {
    const result = await dataApi(actions.USER_INSERT, [firstname, lastname, mobile, email, hashedPassword], request);
    if (result) {
      response.json({success: true});
    } else {
  ...
  try {
    ...
    if (result.length) {
      ...
      if (mailResult.success) {
        const queryResult = await dataApi(actions.USER_REQUEST_RESET_PASSWORD, [token, email], request);
        if (queryResult) {
          response.json({success: true});
        } else {
...
async function validateResetPassword (request, response) {
  ...
  const {email} = request.body;
  const result = await dataApi(actions.USER_VALIDATE_PASSWORD_RESET_TOKEN, [token, email], request);
  if (result.length) {
...
async function resetPassword (request, response) {
  ...
  try {
    const tokenResult = await dataApi(actions.USER_VALIDATE_PASSWORD_RESET_TOKEN, [token, email], request);
    if (tokenResult.length && tokenResult[0].email === email) {
      const hashedPassword = passwordHash.generate(password);
      await dataApi(actions.USER_RESET_PASSWORD, [hashedPassword, email], request);
      response.json({success: true});
    } else {
...
async function fetchProfile (request, response) {
  ...
  try {
    const result = await dataApi(actions.USER_SELECT_PROFILE, [id], request);
    if (result.length) {
...
async function updateProfile (request, response) {
  ...
  try {
    const result = await dataApi(actions.USER_EMAIL_OK_UPDATE, [email, id], request);
    if (result.length) {
      response.json({success: false, error: 'Email already taken.'})
      return;
    }
    await dataApi(actions.USER_UPDATE_PROFILE, [firstname, lastname, email, mobile, id], request);
    response.json({success: true});
  } catch (error) {
    response.json({success: false, error: error.message})
  }
...
async function updatePassword (request, response) {
  ...
  try {
    ...
    const hashedPassword = passwordHash.generate(password);
    await dataApi(actions.USER_UDPATE_PASSWORD, [hashedPassword, id], request);
    response.json({success: true});
  } catch (error) {
    response.json({success: false, error: error.message})
  }
...
async function selectUsers (request, response) {
  try {
    ...
    const users = await dataApi(actions.USERS_SELECT, [], request);
